...
export const getAnswerSystemPrompt = ({ topics, question, userAnswer }: GenerateFeedbackAnswerRequest) => {
  ...
  응답시 주의사항:
  1. 피드백은 긍정적인 부분을 먼저 언급 후, 개선점을 제시해주세요.
  2. 점수가 70점 이상인 경우, 기존 답변 구조를 유지하면서 보완해주세요.
  3. 점수가 70점 미만인 경우에만 완전히 새로운 모범 답안을 제시해주세요.
  4. 꼬리 질문은 답변에서 언급된 개념을 더 깊이 있게 확인하거나 실무 적용 사례를 묻는 질문으로 해주세요.
  5. titleTopic은 반드시 주어진 topics(${topic}) 중에서 선택해주세요.
  6. 답변의 일부 오타는 말로 하는게 아니라 타이핑을 하는 것 이다보니, 심하지 않은 오타는 점수 반영에 포함하지 말아주세요.
  7. 반드시 개선된 답변은 실제 면접에서 높은 평가를 받을 수 있는 수준이어야 합니다.
  8. 반드시 위 JSON 형식을 지켜서 응답해주세요.
  
  부적절한 답변(질문과 무관, 비속어 등)의 경우 score를 0으로 하고 나머지 필드는 기본적인 내용으로 채워주세요.`;
